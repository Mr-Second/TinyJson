...
{
	TEST_LITERAL(json::type::Null, "null");
	TEST_LITERAL(json::type::True, "true");
	TEST_LITERAL(json::type::False, "false");
}

#define TEST_NUMBER(expect, content)\
	do {\
        UT::Json j;\
		j.decode(content, status);\
		EXPECT_EQ_BASE("parse ok", status);\
		EXPECT_EQ_BASE(json::type::Number, j.get_type());\
...
#define TEST_STRING(expect, content)\
	do {\
		...
		v.decode(content, status);\
		EXPECT_EQ_BASE("parse ok", status);\
		EXPECT_EQ_BASE(json::type::String, v.get_type());\
...
static void test_parse_array()
{
	...
	v.decode("[ ]", status);
	EXPECT_EQ_BASE("parse ok", status);
	EXPECT_EQ_BASE(json::type::Array, v.get_type());
	EXPECT_EQ_BASE(0, v.get_array_size());

	v.decode("[ null , false , true , 123 , \"abc\" ]", status);
	EXPECT_EQ_BASE("parse ok", status);
	EXPECT_EQ_BASE(json::type::Array, v.get_type());
	EXPECT_EQ_BASE(5, v.get_array_size());
	EXPECT_EQ_BASE(json::type::Null,   v.get_array_element(0).get_type());
	EXPECT_EQ_BASE(json::type::False,  v.get_array_element(1).get_type());
	EXPECT_EQ_BASE(json::type::True,   v.get_array_element(2).get_type());
	EXPECT_EQ_BASE(json::type::Number, v.get_array_element(3).get_type());
	EXPECT_EQ_BASE(json::type::String, v.get_array_element(4).get_type());
	EXPECT_EQ_BASE(123.0, v.get_array_element(3).get_number());
	EXPECT_EQ_STRING("abc", v.get_array_element(4).get_string());
	
	v.decode("[ [ ] , [ 0 ] , [ 0 , 1 ] , [ 0 , 1 , 2 ] ]", status);
	EXPECT_EQ_BASE("parse ok", status);
	EXPECT_EQ_BASE(json::type::Array, v.get_type());
	EXPECT_EQ_BASE(4, v.get_array_size());
	for(int i = 0; i < 4; ++i) {
        UT::Json a = v.get_array_element(i);
		EXPECT_EQ_BASE(json::type::Array, a.get_type());
		EXPECT_EQ_BASE(i, a.get_array_size());
		for(int j = 0; j < i; ++j) {
            UT::Json e = a.get_array_element(j);
			EXPECT_EQ_BASE(json::type::Number, e.get_type());
...
static void test_parse_object()
{
	...
	v.decode(" { } ", status);
	EXPECT_EQ_BASE("parse ok", status);
	EXPECT_EQ_BASE(json::type::Object, v.get_type());
	...
	EXPECT_EQ_BASE(json::type::Null, v.get_object_value(0).get_type());
	EXPECT_EQ_BASE("f", v.get_object_key(1));
	EXPECT_EQ_BASE(json::type::False, v.get_object_value(1).get_type());
	EXPECT_EQ_BASE("t", v.get_object_key(2));
	EXPECT_EQ_BASE(json::type::True, v.get_object_value(2).get_type());
	EXPECT_EQ_BASE("i", v.get_object_key(3));
	EXPECT_EQ_BASE(json::type::Number, v.get_object_value(3).get_type());
	EXPECT_EQ_BASE(123.0, v.get_object_value(3).get_number());
	EXPECT_EQ_BASE("s", v.get_object_key(4));
	EXPECT_EQ_BASE(json::type::String, v.get_object_value(4).get_type());
	EXPECT_EQ_BASE("abc", v.get_object_value(4).get_string());
	EXPECT_EQ_BASE("a", v.get_object_key(5));
	EXPECT_EQ_BASE(json::type::Array, v.get_object_value(5).get_type());
	EXPECT_EQ_BASE(3, v.get_object_value(5).get_array_size());
	for (int i = 0; i < 3; ++i) {
        UT::Json e = v.get_object_value(5).get_array_element(i);
		EXPECT_EQ_BASE(json::type::Number, e.get_type());
		EXPECT_EQ_BASE(i + 1.0, e.get_number());
	}
	EXPECT_EQ_BASE("o", v.get_object_key(6));
	{
        UT::Json o = v.get_object_value(6);
		EXPECT_EQ_BASE(json::type::Object, o.get_type());
		for(int i = 0; i < 3; ++i) {
            UT::Json ov = o.get_object_value(i);
			EXPECT_EQ_BASE('1' + i, (o.get_object_key(i))[0]);
			EXPECT_EQ_BASE(1, o.get_object_key_length(i));
			EXPECT_EQ_BASE(json::type::Number, ov.get_type());
...
#define TEST_ERROR(error, content) \
	do {\
		...
		EXPECT_EQ_BASE((json::type::Null), v.get_type());\
	} while(0)
...
#define TEST_EQUAL(json1, json2, equality)\
	do {\
        UT::Json v1, v2;\
		v1.decode(json1, status);\
		EXPECT_EQ_BASE("parse ok", status);\
		v2.decode(json2, status);\
		EXPECT_EQ_BASE("parse ok", status);\
...
static void test_copy() {
    UT::Json v1, v2;
	v1.decode("{\"t\":true,\"f\":false,\"n\":null,\"d\":1.5,\"a\":[1,2,3]}");
	v2 = v1;
...
static void test_move() {
	...
	v2 = v1;
	v3 = std::move(v2);
	EXPECT_EQ_BASE(json::type::Null, v2.get_type());
...
static void test_access_boolean()
{
	...
	v.set_string("a");
	v.set_boolean(false);
	EXPECT_EQ_BASE(json::type::False, v.get_type());
}
